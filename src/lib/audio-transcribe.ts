...
/**
 * Petr 2026-05-16: Když Gemini vrátí prázdný `text`, dříve jsme uživateli
 * řekli jen "možná je nahrávka tichá". To je často špatná diagnóza — reálná
 * příčina je vidět v `finishReason` candidatu (MAX_TOKENS / SAFETY / RECITATION
 * / OTHER). Vytáhneme ji a napíšeme konkrétní hlášku co s tím.
 */
function diagnoseEmptyResponse(resp: any, audioByteLength: number, mimeType: string, contextLabel: string): string {
  const sizeMb = (audioByteLength / 1024 / 1024).toFixed(1);
  const candidate = resp?.candidates?.[0];
  const finishReason: string | undefined = candidate?.finishReason;
  const safetyRatings = candidate?.safetyRatings;
  const promptTokens = resp?.usageMetadata?.promptTokenCount;
  const outputTokens = resp?.usageMetadata?.candidatesTokenCount;

  const base = `Audio: ${sizeMb} MB, mime ${mimeType} (${contextLabel}).`;
  const usage = (promptTokens || outputTokens)
    ? ` Tokeny: prompt=${promptTokens ?? "?"}, output=${outputTokens ?? "?"}.`
    : "";

  if (finishReason === "MAX_TOKENS") {
    return `Gemini narazil na limit tokenů (MAX_TOKENS) ještě než stihl vrátit přepis. ${base}${usage} ` +
      `Audio je pravděpodobně moc dlouhé na jeden přepis — zkus rozsekat na kratší úseky (max ~60 min na jeden soubor).`;
  }
  if (finishReason === "SAFETY") {
    const blocked = Array.isArray(safetyRatings)
      ? safetyRatings.filter((r: any) => r?.blocked).map((r: any) => r.category).join(", ")
      : "";
    return `Gemini odmítl přepis z bezpečnostních důvodů (SAFETY${blocked ? `: ${blocked}` : ""}). ${base}${usage} ` +
      `Pokud je obsah neškodný, zkus znovu (občas false positive) nebo nahraj kratší úsek.`;
  }
  if (finishReason === "RECITATION") {
    return `Gemini detekoval, že by přepis kopíroval chráněný obsah (RECITATION). ${base}${usage} ` +
      `Pokud je nahrávka tvoje vlastní, jde o false positive — zkus znovu.`;
  }
  if (finishReason === "OTHER" || finishReason === "BLOCKLIST" || finishReason === "PROHIBITED_CONTENT") {
    return `Gemini ukončil přepis bez vysvětlení (${finishReason}). ${base}${usage} Zkus znovu nebo nahraj v jiném formátu (MP3/WAV).`;
  }
  if (!finishReason || finishReason === "STOP") {
    return `Gemini vrátil prázdný přepis (finishReason=${finishReason ?? "?"}). ${base}${usage} ` +
      `Možné příčiny: tichá nahrávka, neuznaný kodek, nebo transient glitch. Zkus 'Regenerovat' za chvíli.`;
  }
  return `Gemini vrátil prázdný přepis (finishReason=${finishReason}). ${base}${usage}`;
}
